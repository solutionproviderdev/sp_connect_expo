...
const HomeMeetingCard = ({item}) => {
  ...
  return (
    <TouchableOpacity
       className={`
        ${deviceType === 'tablet' ? 'flex-row items-start rounded-xl p-4 mt-4 border border-gray-300 bg-spCardGray' : 
    ...
    >
      ...
      <View className="flex-1 pr-3">
        ...
        <View          
        className={`
          ${deviceType === 'tablet' ? 'flex-row items-center mb-2' : 
            'flex-row items-center mb-'}
        `}>
          <Icon name="account" size={16} color="#6B7280" />
          <Text  
          className={`
            ${deviceType === 'tablet' ? 'font-semibold text-2xl text-gray-800 ml-2' : 
            ...
            <View className="flex-row items-center flex-wrap ml-2">
              {requirements.map((req, index) => (
                <Text
                  ...
                  className={`
                    ${deviceType === 'tablet' ? 'text-white bg-gray-700 px-2 mr-2 rounded-md text-xl' : 
        ...
        <View  
         className={`
          ${deviceType === 'tablet' ? 'flex-row mt-4 gap-4' : 
            'flex-row mt-2 gap-4'}
        `} 
        >
          <TouchableOpacity  
         className={`
          ${deviceType === 'tablet' ? 'flex-1 bg-spGreen py-2 rounded-full' : 
            'flex-1 bg-spGreen py-1 rounded-full'}
        `} 
          >
            <Text  
             className={`
              ${deviceType === 'tablet' ? 'text-white text-center font-bold text-xl' : 
          ...
          <TouchableOpacity  
             className={`
              ${deviceType === 'tablet' ? 'flex-1 bg-spRed py-2 rounded-full' : 
      ...
      <View className="items-end w-1/3">
        ...
        <View className="bg-gray-200 border border-gray-400 mb-1 w-full overflow-hidden rounded-md">
          <Text className=""
          className={`
            ${deviceType === 'tablet' ? 'bg-spDarkGray text-center font-bold px-2 py-0.5 text-white' : 
        ...
        <View className="bg-white border border-gray-400 mb-1 rounded-md overflow-hidden w-full">
          ...
          className={`
            ${deviceType === 'tablet' ? 'bg-spRed text-center font-bold px-2 py-0.5 text-white text-lg' : 
          ...
          <Text  
             className={`
              ${deviceType === 'tablet' ? 'bg-spDarkGray text-center font-medium px-2 py-0.5 text-white text-lg' : 
        ...
        <View className="border border-gray-400 w-full overflow-hidden rounded-md">
          <Text  
           className={`
            ${deviceType === 'tablet' ? 'text-center text-lg font-bold px-2 py-0.5 text-gray-800' : 
          ...
          <Text
          className={`
            ${deviceType === 'tablet' ? 'bg-gray-700 text-center text-lg font-medium px-2 py-0.5 text-white rounded' : 
