        ...
        <View className="relative w-48 h-24">
          <TouchableOpacity
            className={`absolute top-0 left-0 w-12 h-20 ${
              selectedSection === 'A' ? 'bg-spBlue' : 'border-2 border-dashed border-white'
          ...
          <TouchableOpacity
            className={`absolute border-2 border-dashed border-white w-28 h-12 top-8 left-10 ${
              selectedSection === 'B' ? 'bg-spBlue' : 'border-2 border-dashed border-white'
            } rounded-lg flex items-center justify-center`}
  ...
  if (sectionType === 'I Shape') {
    return (
      <View className="mb-3 flex items-center justify-center h-24">
        <View className="relative w-48 h-24">
          <TouchableOpacity
            className={`absolute top-0 left-0 w-36 h-12 ${
              selectedSection === 'A' ? 'bg-spBlue' : 'border-2 border-dashed border-white'
            } rounded-lg flex items-center justify-center`}
            onPress={() => onSelectSection('A')}
          >
      ...
      <View className="mb-3 flex items-center justify-center h-24">
        <View className="relative w-48 h-24">
          <View className="flex-row">
            ...
            <TouchableOpacity
              className={`w-12 h-20 ${
                selectedSection === 'C' ? 'bg-spBlue' : 'border-2 border-dashed border-white'
              } rounded-lg mx-1 flex items-center justify-center`}
